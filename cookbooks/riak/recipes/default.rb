# This portion of the recipe runs on all instances, except the utility instances.  You don't want haproxy on the utility instances but you do on solo|app|app_master & util instances that do not run the database
...
if ['util'].include?(node[:instance_role])
  if node['name'].include?('resque')
    require_recipe "riak::haproxy"
    require_recipe "riak::app_config"
  elsif node['name'].include?('riak_')
